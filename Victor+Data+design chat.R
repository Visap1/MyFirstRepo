...
df_ab%>%  ggplot(aes(reorder(x= countrywb,-sum_urb),y=sum_urb))+
 ...
 # urban growth rate in selected southen African Coutries 

SADIC=df_long %>% filter(countrywb %in% c("Zimbabwe","Zambia","Losotho",
                                          "Malawi","South Africa","Tanzania","Botswana","Eswatini"))
SADIC$year=SADIC$variable
SADIC$country=SADIC$countrywb

SADIC %>% group_by(year,country) %>% summarise(ab=urb_gro) %>% 
  ggplot(aes(year,ab,group=country))+geom_line(aes(color=country))+
  theme_bw()+ylab("urban growth rate")+ggtitle("Urban Growth rate per year in Southern African Countries")+
  scale_x_discrete(breaks = c(2001,2006,2011,2016,2021),labels = c("2001","2006","2011","2016","2021"))+
