...
custom function for adding oods ratios to the regression output
>>>>>>> f608bfd8e11762bc7fb31d06bcf416932cec1db6
```{r}
...
pred_Tmod1 > 0 ~ "Yes"))
=======
mutate(pred_Tmod1 = case_when(pred_Tmod1 <= 0.5 ~"No",
pred_Tmod1 > 0.5 ~ "Yes"))
>>>>>>> f608bfd8e11762bc7fb31d06bcf416932cec1db6
...
Final_titanic_data = Final_titanic_data %>%
mutate(pred_Tmod1_ajusted = predict(Tmod1)) %>%
mutate(pred_Tmod1_ajusted = case_when(pred_Tmod1_ajusted <= 0 ~"No",
pred_Tmod1_ajusted > 0 ~ "Yes"))
...
Final_titanic_data = Final_titanic_data %>%
  mutate(correct_prediction_Ajusted=case_when(pred_Tmod1_ajusted == Survived  ~ "correct",
                                      pred_Tmod1_ajusted != Survived ~ "incorrect"))
...
group_by(correct_prediction_Ajusted) %>%
summarise(count = n()) %>%
mutate(freq = count / sum(count))%>%kable(caption="Correctly Predicted Cases Yes",digits = 2)
