...
surgery_data_A=surgery_data_A%>%
mutate(pred_int =predict(mod_RI),pred_slope =predict(Mod2_RS))
...
```{r}
# random slope 
ggplot(surgery_data_A,aes(y = pain,x =cortisol_serum ,
group =hospital))+geom_point(size =4,colour="blue")+geom_line(color ="red",size=1,
aes(y =pred_slope,x =cortisol_serum ))+facet_wrap(~hospital,ncol =5)+
  theme_bw()
...
```{r message=FALSE,results='asis',fig.show='asis'}

ggplot(surgery_data_A,aes(y = pain,x =cortisol_serum ,
group =hospital))+geom_point(size =5,colour="red")+geom_line(color ="blue",size=1,
aes(y =surgery_data_A$pred_int,x =cortisol_serum ))+facet_wrap(~hospital,ncol =5)+
  theme_bw()
```
